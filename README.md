...
* **What**: Some tricks to train RNN and seq2seq models:
   * Embedding size: 1024 or 620. Lower dimensionality like 256 can also lead to good performances. Higher does not necessarily lead to better performances
   ...
   * Using beam (predicting multiple candidate instead of greedy approach) leads to better result if the beam size and length penalty are carefully fine tuned. Authors use beam size of 10 with length penalty of 1.0

  **Ref**: *Massive Exploration of Neural Machine Translation Architectures, Denny Britz, Anna Goldie et al.* (https://arxiv.org/abs/1703.03906)
...
* **What**: Experience Replay: Instead of updating every frames as the agent plays, to avoid correlations between the frames, it's better to sample a batch in the history of the transition taken (state, actionTaken, reward, nextState). This is basically the same idea as shuffling the dataset before training for supervised tasks. Some strategies exist to sample batches which contain more information (in the sense predicted reward different from real reward).<br />
  **Ref**: *Prioritized Experience Replay, Tom Schaul et al.* (https://arxiv.org/abs/1511.05952)

* **What**: PAAC (Parallel Advantage Actor Critic): It's possible to simplify the the A3C algorithm by batching the agent experiences and using a single model with synchronous updates.<br />
  **Ref**: *Efficient Parallel Methods for Deep Reinforcement Learning, Alfredo V. Clemente et al.* (https://arxiv.org/abs/1705.04862v2)
